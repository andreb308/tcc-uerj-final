 ...
 */
function fromGenius(title: string, artistName: string, signal?: AbortSignal): Promise<string> {
 ...
 */
function fromAZLyrics(title: string, artistName: string, signal?: AbortSignal): Promise<string> {
  ...
  return fetchHtml(url, {}, signal).then(($) => {
 ...
 */
function fromLetras(title: string, artistName: string, signal?: AbortSignal): Promise<string> {
 ...
 */
function fromLyricsCom(title: string, artistName: string, signal?: AbortSignal): Promise<string> {
  const searchUrl =
 ...
 */
function fromParolesNet(title: string, artistName: string, signal?: AbortSignal): Promise<string> {
 ...
 */
export function findLyrics(title: string, artistName: string): Promise<string> {
  ...
  if (/\(.*\)/.test(title) || /\[.*\]/.test(title)) {
    ...
    promises.push(
      runDelayed(GENIUS_ADVANTAGE_MS, signal, () =>
        findLyrics(cleanTitle, artistName).then((l) => ({
  ...
  if (primaryArtist && primaryArtist.length > 1 && primaryArtist !== artistName) {
    promises.push(
      runDelayed(GENIUS_ADVANTAGE_MS, signal, () =>
        findLyrics(title, primaryArtist).then((l) => ({
